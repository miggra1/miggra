...
export default async function TimelineDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <main className="min-h-screen"><article className="mx-auto max-w-2xl px-6 py-16">
      <Link href="/timeline" className="text-[13px] text-[var(--subtle)] hover:text-[var(--fg)] transition mb-10 inline-block">← 时间线</Link>
      <span className="text-[13px] text-[var(--fg-secondary)]">{item.year} · {item.kind === "PERSONAL" ? "人生节点" : "站点节点"}</span>
      <h1 className="text-[32px] font-medium tracking-tight mt-2 mb-10">{item.title}</h1>
