...
const sectionMeta: Record<ContentSection, Omit<SliceSection, "items" | "key">> = {
  NOW: { title: "Now", eyebrow: "Current", description: "最近正在做、正在想、正在靠近的事。", hrefBase: "/now" },
  WISH: { title: "愿望", eyebrow: "Wish", description: "还没完成也没关系，先把想去的方向放在这里。", hrefBase: "/wish" },
  READING: { title: "书单", eyebrow: "Reading", description: "读过、在读和想读的书，慢慢留下痕迹。", hrefBase: "/reading" },
  INSPIRATION: { title: "灵感", eyebrow: "Ideas", description: "一些闪过的小想法，先收住，以后再长大。", hrefBase: "/inspirations" },
  TIMELINE: { title: "时间线", eyebrow: "Timeline", description: "更完整的节点已经单独放在时间线页面。", hrefBase: "/timeline" },
};

function fallbackFor(section: ContentSection): SliceItem[] {
  if (section === "NOW") return fallbackNowItems.map((detail, index) => ({ title: `状态 ${index + 1}`, detail, meta: "Current" }));
...
export default async function LifePage() {
  const sections = await Promise.all([getSection("NOW"), getSection("WISH"), getSection("READING"), getSection("INSPIRATION")]);
      ...
      <div className="mx-auto max-w-5xl px-5 py-14 sm:px-6 sm:py-16">
        ...
        <div className="mt-8 flex gap-2 overflow-x-auto pb-2">
          {sections.map((section) => (
            <a key={section.key} href={`#${section.key.toLowerCase()}`} className="shrink-0 rounded-full border border-[var(--border)] bg-[var(--card)] px-4 py-2 text-sm text-[var(--muted)] transition hover:bg-[var(--card-strong)] hover:text-[var(--fg)]">
              {section.title}
            </a>
          ))}
        </div>
          ...
          {sections.map((section) => (
            <section key={section.key} id={section.key.toLowerCase()} className="scroll-mt-24">
              <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
                ...
                <Link href={section.hrefBase} className="text-sm text-[var(--subtle)] transition hover:text-[var(--fg)]">查看全部</Link>
              </div>
              ...
              <div className="grid gap-4 md:grid-cols-2">
                {section.items.map((item, index) => {
                  const content = (
                    <article className="h-full rounded-[1.35rem] border border-[var(--border)] bg-[var(--card)] p-5 transition hover:bg-[var(--card-strong)]">
                      <div className="flex items-start justify-between gap-4">
                        ...
                        {(item.status || item.meta) && <span className="shrink-0 rounded-full border border-[var(--border)] px-2.5 py-1 text-xs text-[var(--subtle)]">{item.status ?? item.meta}</span>}
                      </div>
                  ...
                  return item.href ? <Link key={item.href} href={item.href} className="block">{content}</Link> : <div key={`${section.key}-${index}`}>{content}</div>;
                })}
              </div>
            </section>
          ))}
