...
const tone = sectionIdentity.now;
    ...
    <main className="min-h-screen bg-[var(--bg)] text-[var(--fg)] ambient-bg">
      <div className="mx-auto max-w-3xl px-6 py-16">
        <header className="mb-12">
          <p className={`text-sm uppercase tracking-[0.3em] ${tone.eyebrowClass}`}>Now</p>
        ...
        {metas.length > 1 && (
          <div className="mb-8 flex flex-wrap gap-2">
            <Link href={hrefForMeta()} className={`rounded-full border px-4 py-2 text-sm transition ${!selectedMeta ? `${tone.badgeClass}` : "border-[var(--border)] bg-[var(--card)] text-[var(--muted)] hover:text-[var(--fg)]"}`}>全部</Link>
            {metas.map((meta) => (
              <Link key={meta} href={hrefForMeta(meta)} className={`rounded-full border px-4 py-2 text-sm transition ${selectedMeta === meta ? `${tone.badgeClass}` : "border-[var(--border)] bg-[var(--card)] text-[var(--muted)] hover:text-[var(--fg)]"}`}>{meta}</Link>
            ))}
          </div>
        )}
          ...
          {filtered.map((item, i) => {
            const content = (
              <div className={`rounded-[1.5rem] border bg-[var(--card)] p-6 card-interactive animate-in ${tone.borderClass} ${tone.hoverClass}`} style={{ animationDelay: `${i * 60}ms` }}>
                <p className={`text-xs uppercase tracking-[0.2em] ${tone.eyebrowClass}`}>{item.meta}</p>
