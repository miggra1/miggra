  ...
  return (
    <>
      ...
      <main className="relative min-h-screen w-full max-w-[100vw] overflow-hidden bg-[var(--bg)] text-[var(--fg)]">
        ...
        <section className="relative mx-auto grid min-h-[calc(100svh-56px)] w-full max-w-6xl gap-10 px-5 pb-14 pt-14 sm:px-6 sm:py-16 lg:min-h-screen lg:grid-cols-[0.95fr_1.05fr] lg:items-center lg:px-10">
          <div className="min-w-0">
            ...
            <div className="mt-8 space-y-4 sm:space-y-5">
              <p className="text-xs uppercase tracking-[0.32em] text-[var(--subtle)] sm:text-sm sm:tracking-[0.35em]">Miggra Notes</p>
              <h1 className="max-w-full text-[2.55rem] font-semibold leading-[1.14] tracking-normal sm:max-w-4xl sm:text-6xl sm:leading-tight sm:tracking-tight sm:text-balance lg:text-7xl">
                ...
                <span className="hidden sm:inline">先写下那些细小、真实、会回来的瞬间。</span>
              </h1>
              <p className="max-w-[21rem] text-[15px] leading-7 text-[var(--muted)] sm:max-w-2xl sm:text-lg sm:leading-8">
                这个站的核心不是展示很多功能，而是把日常里的想法、情绪和片刻先留下来。照片、书单和愿望都围绕这些文字慢慢展开。
              </p>
            </div>

            <div className="mt-8 flex flex-wrap gap-3 sm:gap-4">
              <Link href="/notes" className="rounded-full bg-[var(--accent)] px-5 py-3 text-sm font-medium text-[var(--accent-fg)] transition hover:opacity-90 sm:px-6">
                进入碎碎念
              </Link>
              {featured ? (
                <Link href={`/notes/${featured.id}`} className="rounded-full border border-[var(--border)] bg-[var(--card)] px-5 py-3 text-sm font-medium text-[var(--fg)] backdrop-blur-xl transition hover:bg-[var(--card-strong)] sm:px-6">
                  读精选
                </Link>
              ) : null}
              <Link href="/about" className="rounded-full border border-[var(--border)] bg-[var(--card)] px-5 py-3 text-sm font-medium text-[var(--fg)] backdrop-blur-xl transition hover:bg-[var(--card-strong)] sm:px-6">
                关于这里
              </Link>
            </div>
          </div>
          ...
          <div className="grid min-w-0 gap-4">
            {featured ? (
              <Link href={`/notes/${featured.id}`} className="group rounded-[1.5rem] border border-[var(--border)] bg-[var(--card)] p-6 shadow-2xl shadow-black/30 transition hover:bg-[var(--card-strong)] sm:p-7">
                {featured.coverImage ? (
                  <div className="mb-5 overflow-hidden rounded-2xl">
                    <img src={featured.coverImage} alt="" className="h-52 w-full object-cover transition duration-700 group-hover:scale-[1.03] sm:h-64" />
                  </div>
                ) : null}
                <div className="flex flex-wrap items-center gap-2 text-xs text-[var(--subtle)]">
                  ...
                  {featured.pinned ? <span className="rounded-full border border-amber-300/25 px-3 py-1 text-amber-100/80">置顶</span> : null}
                </div>
                <p className="mt-5 text-xs uppercase tracking-[0.28em] text-[var(--subtle)]">Featured note</p>
                <h2 className="mt-2 text-3xl font-semibold leading-tight">{featured.title}</h2>
                <div className="mt-4 line-clamp-5">
                  <MarkdownRenderer preview>{featured.text}</MarkdownRenderer>
                </div>
                <p className="mt-5 text-sm text-[var(--subtle)] transition group-hover:text-[var(--fg)]">继续读 →</p>
              </Link>
            ) : null}

            <div className="grid gap-4 sm:grid-cols-2">
              <div className="rounded-[1.35rem] border border-[var(--border)] bg-[var(--card)] p-5 backdrop-blur-xl">
                <p className="text-sm text-[var(--subtle)]">写作状态</p>
                <p className={`mt-2 text-xl font-semibold leading-8 ${status.tone === "fresh" ? "text-emerald-400" : status.tone === "warm" ? "text-[var(--fg)]" : "text-[var(--muted)]"}`}>
                  {status.text}
                </p>
              ...
              {random ? (
                <Link href={`/notes/${random.id}`} className="rounded-[1.35rem] border border-[var(--border)] bg-[var(--card)] p-5 transition hover:bg-[var(--card-strong)]">
                  ...
                  <p className="mt-3 line-clamp-4 text-sm leading-7 text-[var(--fg)]">
                    {random.text.length > 110 ? `${random.text.slice(0, 110)}...` : random.text}
                  </p>
                </Link>
              ) : null}
            </div>
            ...
            <section>
              <div className="mb-5 flex items-end justify-between gap-4">
                <div>
                  ...
                  <h2 className="mt-2 text-2xl font-semibold">最近写下的碎碎念</h2>
                  <p className="mt-2 text-sm leading-7 text-[var(--muted)]">这里是网站的主入口，也是最值得回看的部分。</p>
                </div>
                <Link href="/notes" className="text-sm text-[var(--subtle)] transition hover:text-[var(--fg)]">全部</Link>
              </div>
              <div className="grid gap-4 lg:grid-cols-3">
                {latest.slice(0, 6).map((note) => (
                  <Link key={note.id} href={`/notes/${note.id}`} className="rounded-[1.35rem] border border-[var(--border)] bg-[var(--card)] p-5 transition hover:bg-[var(--card-strong)]">
                    {note.coverImage ? (
                      <div className="mb-4 overflow-hidden rounded-2xl">
                        <img src={note.coverImage} alt="" className="h-36 w-full object-cover" />
                      </div>
                    ) : null}
              ...
              <section>
                <div className="mb-5 flex items-end justify-between gap-4">
                  <div>
                    ...
                    <h2 className="mt-2 text-2xl font-semibold">照片是文字旁边的光</h2>
                  </div>
            ...
            <section>
              <div className="mb-5 flex flex-wrap items-end justify-between gap-4">
                <div>
                  ...
                  <h2 className="mt-2 text-2xl font-semibold">其它栏目围绕文字展开</h2>
                  <p className="mt-2 max-w-2xl text-sm leading-7 text-[var(--muted)]">
                    Now、愿望、书单和灵感都保留独立入口，但它们更像碎碎念之外的补充材料。
                  </p>
                </div>
                <Link href="/life" className="text-sm text-[var(--subtle)] transition hover:text-[var(--fg)]">生活切片</Link>
              </div>
